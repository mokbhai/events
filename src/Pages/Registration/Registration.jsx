...
const Registration = () => {
  ...
  const handleEventSelection = (eventId, category) => {
    setSelectedEvents((prev) => ({ ...prev, [category]: eventId }));
  };
  ...
  const getSelectedEventNames = () => {
    return Object.entries(selectedEvents)
      .map(([category, eventId]) => {
        const event = events.find((e) => e._id === eventId); // Find the event by ID
        if (event) {
  ...
  const calculateTotalAmount = () => {
    ...
    const totalEventAmount = selectedEventIds.reduce((sum, eventId) => {
      const event = events.find((e) => e._id === eventId);

      // Convert amount to a number before adding
      const eventAmount = event
        ? parseFloat(event.registrationCharge.amount)
        : 0;
      return sum + eventAmount;
    }, 0);

    // Calculate the total accommodation fee based on team members opting for accommodation
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < teamDetails.teamMembers.length; i++) {
      ...
      if (isMemberFilled) {
        ...
        for (const eventId of selectedEventIds) {
          const event = events.find((e) => e._id === eventId);
          if (event) {
            const memberClass = parseInt(member.class, 10); // Parse class to integer
    ...
    const totalEventAmount = calculateTotalAmount();
    const totalAmount = totalEventAmount;
  ...
  return (
    <div className="font-avenger flex flex-col text-slate-300 min-h-screen bg-gray-900">
      <Navbar />
      <form
      ...
      >
          ...
          <div className="md:w-1/2 ">
            <Summary
              ...
              totalAccommodationFee={
                teamDetails.teamMembers.filter(
                  (member) => member.accommodationOpted
        ...
        <div className="w-full  items-center mt-6 flex justify-center opacity-80 gap-2">
          <button
            ...
            type="clear"
            className="bg-transparent border text-white w-1/3 text-lg self-center font-bold py-2 rounded-3xl hover:bg-red-600 hover:font-extrabold "
          >
            Clear Details
          </button>
          <button
