      ...
      <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between px-4 bg-slate-400 font-sans">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between w-full">
          ...
          <div className="flex items-center gap-x-3 sm:mb-0 px-2">
            ...
            <img className="w-32 h-auto" src={logo} alt="TechSprint" />
          </div>
      ...
      <div className="w-full px-4 bg-slate-400 ">
        ...
        >
          {logos.map((logo, index) => (
            <img key={index} className="h-16 logo" src={logo.src} alt={logo.alt} />
          ))}
        </div>
      </div>
    </nav>
...
const Navbar = () => {
  ...
  return (
    <nav>
      {" "}
      {/* Wrap with a <nav> element for semantic correctness */}
      {isMobileNav ? (
        <MobileNav /> // No need to pass toggleMenu and showMenu if handled internally within MobileNav
      ) : (
        <DekstopNav />
      )}
    </nav>
